...
if(isset($_POST['send'])){
  // Novi slog	
    ...
    if($_POST['f_ime']=="" and $_POST['f_prezime']=="" and $_POST['f_dat_rodjenja']=="" and $_POST['f_sif_kluba']==""){
			...
			//Dodavanje sloga 
			$sql = "INSERT INTO korisnik (`sif_korisnik`, `ime_i`, `prezime_i`, `dat_rodjenja_i`, `sif_pl_org`)
  ...
  //Izmene sloga
    ...
    $sql = "UPDATE korisnik SET ime_i='".$_POST['f_ime']."', prezime_i='".$_POST['f_prezime']."', dat_rodjenja_i='".$_POST['f_dat_rodjenja']."', sif_pl_org=".$_POST['f_sif_pl_org']." WHERE sif_korisnik = ".$_POST['f_sif_korisnika'].";";
  ...
  //Brisanje sloga
    ...
    $sql="DELETE FROM korisnik WHERE sif_korisnik=".$_POST['f_sif_korisnika'].";";
...
if(isset($_GET['akcija'])) {
  ...
  if($_GET['akcija']=="edit" or $_GET['akcija']=="del"){
		$sql="SELECT * FROM korisnik WHERE sif_korisnik=".$_GET['sif'];
		$result = mysqli_query($conn, $sql) or die (mysqli_error($conn)." $sql");
		$row = mysqli_fetch_assoc($result);
		$data["sif_korisnik"] = $row['sif_korisnik'];
		$data["ime"] = $row['ime_i'];
...
?>  
  <!-- Formular -->
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
		...
		<input type="hidden" name="f_sif_korisnika" value="<?php echo $data["sif_korisnik"]; ?>"><br>
...
<?php  
  ...
  if (isset($_GET['sort'])){
		...
		$sort = "sif_korisnik desc";
	}
  
  $sql="select *, concat(naz_pl_org,' ',mesto_pl_org) as klub ";
	$sql.="from korisnik ";
	$sql.="INNER JOIN pl_org on korisnik.sif_pl_org = pl_org.sif_pl_org ";
  $sql.="order by ".$sort;
  ...
  <table border=1>
      ...
      <th> </th>
		...
		foreach($result as $row){
			echo "<tr>";
        echo "<td>".$row['sif_korisnik']."</td>";
				echo "<td>".$row['ime_i']."</td>";
				echo "<td>".$row['prezime_i']."</td>";
        echo "<td>".$row['dat_rodjenja_i']."</td>";
				echo "<td>".$row['klub']."</td>";
        echo "<td><a href='". $_SERVER['PHP_SELF']."?akcija=edit&sif=".$row['sif_korisnik']."' class=btn>Izmeni</a></td>";
				
				echo "<td><a href='". $_SERVER['PHP_SELF']."?akcija=del&sif=".$row['sif_korisnik']."' class=btn>Brisanje</a></td>";
			echo "</tr>";	
		}
	echo "</table>";
